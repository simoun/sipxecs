...
public class UserPermissionsResource extends UserResource {
    ...
    private UserPermissionRestInfoFull createUserPermissionRestInfo(int id) {
        User user = getCoreContext().loadUser(id);
    ...
    private List<User> getUsers(Collection<Integer> userIds) {
        ...
        for (int userId : userIds) {
            users.add(getCoreContext().loadUser(userId));
        }

        return users;
    }

    private List<User> getUsersByBranch(int branchId) {
        ...
        for (User user : users) {
        	if (user.getBranch().getId().intValue() == branchId) {
